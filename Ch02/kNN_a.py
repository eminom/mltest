...
# The k shall neither be too large nor too small
# Too Small: Lack of comparions
# Too Large: Too many targets are alike.
# in:k: How many neighbours are taken into account.
...
def createDataSet():
    group = array([[1.0,1.1],[1.0,1.0],[0,0],[0,0.1]
			...
			, [0.03, 0.2]
			,[0.2,0.07]])
    assert len(group.shape) == 2, "Two dimensioned"
    labels = ['A','A','B','B','A', 'A','B','B']
    assert len(labels) == group.shape[0]
    return group, labels

def showDataSet(dataSet, kind):
	def randomColor():
		color = ('red', 'blue', 'yellow', 'green', 'purple', 'cyan')
		return color[int(random.uniform(0, len(color)))]
	colorDC = {}
	fig = plt.figure()
	...
	for i in range(dataSet.shape[0]):
		...
		if not kind[i] in colorDC:
			while True:
				c = randomColor()
				if not c in colorDC.values():break
			colorDC[kind[i]] = c
		assert kind[i] in colorDC, "We got this color"
		colors.append(colorDC[kind[i]])
